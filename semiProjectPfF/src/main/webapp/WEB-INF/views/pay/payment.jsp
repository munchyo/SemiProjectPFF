	...
	<section id="payment">
		<div id="pointInfo">
			...
			<table class="table marginTopMediumLarge">
				...
				<tr scope="col">
					<td width="15%" class="b">가용적립금: ${ loginUser.point }원</td>
					<td class="b">쿠폰: 0개</td>
				</tr>
			</table>
		</div>
...
<%-- 					<c:set var="pointSum" value="0" scope="page"/> --%>
					...
					<c:forEach items="${ productList }" var="product" varStatus="pStatus">
						<c:set var="productInfo" value="${productInfo}${ product.productNo }${ delimiterS }${ count[pStatus.index] }${ delimiterS }${ size[pStatus.index] }${ delimiterS }${ delimiterA }"/>
						<c:set var="eachSum" value="${ product.productPrice*(100-product.discountRate)/100 * count[pStatus.index] }" />
						<c:set var="originalSum" value="${ originalSum + product.productPrice*count[pStatus.index] }" />
						...
						<tr>
							<td width="15%"><img src="${pageContext.request.contextPath}/resources/image/coat.jpg" alt="상품 사진" width="80px"></td>
							<td width="20%">
								<div class="itemName">${ product.productName }</div> 
								<div class="itemOption marginTopMicro">[옵션: ${ size[pStatus.index] }]<br/>
								</div>
							</td>
							<td width="15%">
							...
							<td width="5%">${ count[pStatus.index] }</td>
